...
class Demo():
    def __init__(self, args):
        ...
        self.export_obj = args.export_obj
        ...
        ## styleGAN 
        self.gan_ckpt_path = args.checkpoint_gan
        self.channel_multiplier = 1
        self.gan_size = 64
        self.n_interpolation = 8
        self.truncation_mean = 4096
        self.z_dim = 512
        self.truncation = 0.7
        self.F1_d = 2
        self.n_mlp = 8
        self.generator = Generator(128, self.z_dim, self.n_mlp, channel_multiplier=self.channel_multiplier)
        ...
        if self.render_video:
            from factorGAN2.renderer import Renderer
    ...
    def run(self):
        ...
        with torch.no_grad():
            latent_z = torch.randn(1, self.z_dim, device=self.device)
            latent_w = self.generator.style_forward(latent_z)
            latent_w = self.mean_latent + self.truncation * (latent_w - self.mean_latent)

            self.input_im, _ = self.generator([latent_w], truncation=self.truncation, truncation_latent=self.mean_latent, input_is_w=True)
            self.input_im = torch.nn.functional.interpolate(self.input_im, (self.image_size, self.image_size), mode='bilinear', align_corners=False)
            ...
            self.recon_im = nn.functional.interpolate(self.recon_im, (self.output_size, self.output_size), mode='bilinear', align_corners=False)
    ...
    def save_results(self, save_dir):
        ...
        save_image(save_dir, self.recon_im[0].clamp(-1,1)/2+0.5, 'recon_image')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_gan', default='./checkpoints/stylegan2-celeba-config-e.pt', type=str, help='Path to the gan checkpoint')
    parser.add_argument('--output_size', default=64, type=int, help='Output image size')
    parser.add_argument('--output_num', default=16, type=int, help='number of output')
    ...
    parser.add_argument('--export_obj', default=False, action='store_true', help='export obj')
    ...
    for i in range(args.output_num):
        output = model.run()
        save_dir = f'{args.result}/{str(i).zfill(6)}'
